...
class Plates(Morphling):
	...
	def char_expansion(graph, u1 = None):
		...
		if graph.is_L_or_R(n1) or graph.is_L_or_R(u1):
			# New nodes:
			v1 = graph.add_ran_node(reflect=True)
			...
			# New nodes:
			v1 = graph.add_ran_node(middle=True)
...
def main(argv):
	...
	morph, move = G1.mutator(Node=argv[2], move=int(argv[3]))
	print(move)
