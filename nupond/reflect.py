...
def Check_Symmetry(G):
    """ For each row, check if there is a reflection of it elsewhere in the data """
...
def main(argv):
    with open(argv[1]) as f:
        G = f.readlines()
    G_list = [x.strip().split(' ') for x in G]
    G_set = set( frozenset(element) for element in G_set )
