...
""" Build parsimony distance matrix using simulated annealing """

__appname__ = 'SA_dist.py'
__author__ = 'Tristan JC (tjc19@ic.ac.uk)'
__version__ = '0.0.1'

## imports ##
import sys
from GrEvolution import SA_GrEv
import pandas as pd
import os
...
def GrEvo_job(G1, G2, G1_name, G2_name, Climb_repeats):
	...
	while Gen < Climb_repeats:
		Gen += 1
		temp, parsimony = SA_GrEv(G1, G2, goal = best_parsimony+1, G1_name=G1_name, G2_name=G2_name)
		
		if parsimony < best_parsimony:
			best_parsimony = parsimony
	
	#os.system('play -nq -t alsa synth {} sine {}'.format(0.1, 250))
	print(G1_name + " X " + G2_name + ": " + str(best_parsimony))
	return [G1_name, G2_name, best_parsimony]

def dist_matrix(G_Data, C_Data, Comp_Data, repeats, file, num_cores, mode="simple", overwrite=False):
	...
	if mode == "parallel":
		...
		if overwrite:
			Results = Parallel(n_jobs=num_cores)(delayed(GrEvo_job)(Graphs[i[0]], Graphs[i[1]], i[0], i[1], repeats) for i in combins)
			for i in Results:
				if Pars_table.loc[i[0], i[1]] > i[2]:
					Pars_table.loc[i[0], i[1]] = i[2]
					Pars_table.loc[i[1], i[0]] = i[2]
		else:
			Results = Parallel(n_jobs=num_cores)(delayed(GrEvo_job)(Graphs[i[0]], Graphs[i[1]], i[0], i[1], repeats) for i in combins if not Pars_table.loc[i[0], i[1]])
			for i in Results:
				Pars_table.loc[i[0], i[1]] = i[2]
				Pars_table.loc[i[1], i[0]] = i[2]
