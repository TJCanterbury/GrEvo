...
def GrEvAl(G1, G2, repeat = 20, source=[["body", "body"]]):
	...
	for i in range(repeat):
		if G1.number_of_nodes() > G2.number_of_nodes() or G1.graph['completeness'] + G2.graph['completeness'] != 2:
			score, aln = ran_align(G2, G1, source)
			G2.graph["Align"] = aln
		else:
			score, aln = ran_align(G1, G2, source)
			G1.graph["Align"] = aln
		if score > best_score:
			best_score = score
			best_aln = aln
