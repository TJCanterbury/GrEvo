...
class Morphling(nx.Graph):
	...
	def char_squeein(graph, node=None):
		...
		#reflect
		r_node = reflect_n(node)
		r_neighb1 = reflect_n(neighb1)
...
def measurer(G1, G2):
	if nx.is_isomorphic(G1,G2):
		return 0
	score, aln = Align(G1.copy(), G2.copy())
	score = 1 - score
	score2, rad = LCN4(G1, G2)
	score = np.mean((score2, score))
	return score

### Evo (hill climb) algorithm ###
def perturber(G1, move = None, Node = None, stuck = False):
	""" Make random move """
	movement = None
	try_again = True
