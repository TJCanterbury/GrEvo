...
def climber(G1, G2, sample_size = 100, file = "test.txt", attempts = 10):
    ...
    moves2.append("G2 " + recorder(move, old_score))
    dead_end1 = []
    dead_end2 = []
    size = sample_size
    ...
    while old_score != 0:
        ...
        best_score, best_move, best_G = searcher(g1s[Generation], g2s[Generation], size, attempts, dead_end1)
        best_score2, best_move2, best_G2 = searcher(g2s[Generation], best_G, size, attempts, dead_end2)

        if best_move and best_move2:
            Generation += 1
            size = sample_size * ( Generation + 1 )
            drawx(best_G, "display_1.png")
            drawx(best_G2, "display_2.png")
            g1s.append(best_G.copy())
            g2s.append(best_G2.copy())
            moves1.append("G1 " + recorder(best_move, best_score))
            moves2.append("G2 " + recorder(best_move2, best_score2))
            ...
            dead_end2.append(best_G2)
        
    return Generation, moves1, moves2
...
def main(argv):
    ...
    if argv[3] == "manual":
        ...
        Generations, moves1, moves2 = climber(G1, G2, sample_size=breadth, file=Results_file, attempts=attempts)

        n_moves = Generations * 2
        print(n_moves)
        moves = [moves1, moves2]
